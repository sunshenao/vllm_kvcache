...
class SimpleConnector(KVConnectorBase):
    ...
    def recv_kv_caches_and_hidden_states(
        ...
        if not bypass_model_exec:
            ...
            print("失败",'-=-='*10)
            raise ValueError("bypass_model_exec is False, transfered is failed")
    ...
    def thread_send_kv_caches_and_hidden_states(
        ...
        for idx, slen in enumerate(seq_lens):
            # send(idx,slen)
            thread_list.append(threading.Thread(target=send,args=(idx,slen)))
            thread_list[-1].start()
    ...
    def thread_recv_kv_caches_and_hidden_states(
        ...
        for idx, slen in enumerate(seq_lens):
            ...
            thread_list.append(threading.Thread(target=recv,args=(idx,slen)))
            thread_list[-1].start()
    ...
    def aprocess_send_kv_caches_and_hidden_states(
        self,
        model_executable: torch.nn.Module,
        model_input: "ModelInputForGPUWithSamplingMetadata",
        kv_caches: List[torch.Tensor],
        ...
        for idx, slen in enumerate(seq_lens):
            ...
            thread_list.append(multiprocessing.Process(target=send,args=(idx,slen)))
            thread_list[-1].start()
    ...
    def aprocess_recv_kv_caches_and_hidden_states(
        self, model_executable: torch.nn.Module,
        model_input: "ModelInputForGPUWithSamplingMetadata",
        kv_caches: List[torch.Tensor]
    ) -> Tuple[Union[torch.Tensor, IntermediateTensors], bool,
        ...
        for idx, slen in enumerate(seq_lens):
            ...
            thread_list.append(multiprocessing.Process(target=recv,args=(idx,slen)))
            thread_list[-1].start()
        ...
        def async_send():
            ...
            with torch.cuda.stream(send_stream):
                ...
                for idx, slen in enumerate(seq_lens):
                    ...
                    all_keys.append(keys.unsqueeze(0))  # 添加批次维度
                    all_values.append(values.unsqueeze(0))  # 添加批次维度
                    all_current_tokens.append(current_tokens)

                # 批量插入
                for idx, (current_tokens, keys, values) in enumerate(zip(all_current_tokens, all_keys, all_values)):
                    self.insert(current_tokens,
                                torch.ones_like(current_tokens, dtype=bool),
                                keys.squeeze(0), values.squeeze(0),
                                hidden_or_intermediate_states[start_pos:end_pos])
        ...
        def async_recv():
            ...
            with torch.cuda.stream(recv_stream):
                ...
                for idx, slen in enumerate(seq_lens):
                    ...
                    for i in range(model_executable.model.start_layer, model_executable.model.end_layer):
                        kv_cache = kv_caches[i - model_executable.model.start_layer]
                        layer = model_executable.model.layers[i]
                ...
                if not bypass_model_exec:
                    logger.debug("[rank%d]: Failed to receive all KVs and hidden states, redo model forwarding.", torch.distributed.get_rank())
                    hidden_or_intermediate_states = None
                    print("失败", '-=-=' * 10)
                else:
                    logger.debug("[rank%d]: Successfully received all KVs and hidden states, skip model forwarding.", torch.distributed.get_rank())
                    hidden_or_intermediate_states = torch.cat(all_hidden, dim=0)

            return (hidden_or_intermediate_states, bypass_model_exec, model_input)

        # 启动异步线程
        return async_recv()
        # thread = threading.Thread(target=async_recv)
        # thread.start()
        # thread.join()
