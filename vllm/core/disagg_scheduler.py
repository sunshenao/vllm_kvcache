...
class Scheduler:
    ...
    def __init__(
        ...
        self.transfered_con = transfered_con
    ...
    def _add_seq_group_to_transfered(self, seq_group: SequenceGroup) -> None:
        # Add sequence groups to the swapped queue.
        # Only for testing purposes.
        self.transfered.append(seq_group)
    ...
    def abort_seq_group(self, request_id: Union[str, Iterable[str]]) -> None:
        ...
        for state_queue in [self.waiting, self.transfered, self.running, self.swapped]:
    ...
    def has_unfinished_seqs(self) -> bool:
        return len(self.waiting) != 0 or len(self.running) != 0 or len(
            self.swapped) != 0 or len(self.transfered) != 0
            ...
            while not self._can_append_slots(seq_group, enable_chunking):
                ...
                if do_preempt:
                    ...
                    if preempted_mode == PreemptionMode.RECOMPUTE:
                        # victim_seq_group.is_decode = False
                ...
                is_prefill = seq_group.is_prefill() # if not self.is_decode() else False
    ...
    def _schedule_prefills(
        ...
        while self._passed_delay(time.time()) and waiting_queue:
            ...
            if (budget.num_batched_tokens
                ...
                print(budget.num_batched_tokens,self.scheduler_config.max_num_batched_tokens)
                break
    ...
    def _shedule_disagg_transfer(self,transfer_data=False,transfer_queue=None):
        budget = SchedulingBudget(
            token_budget=self.scheduler_config.max_num_batched_tokens,
            max_num_seqs=self.scheduler_config.max_num_seqs,
        )
        for seq_group in (self.running + self.transfered):
            budget.add_num_seqs(seq_group.request_id,
                                seq_group.get_max_num_running_seqs())
        curr_loras = set(
            seq_group.lora_int_id for seq_group in (self.running + self.transfered)
            if seq_group.lora_int_id > 0) if self.lora_enabled else None

        transfered = SchedulerPrefillOutputs.create_empty()
        

        if not self.swapped:
            transfered = self._schedule_transfered(budget,
                                            curr_loras,
                                            enable_chunking=False,
                                            transfer_data=transfer_data)

        if len(transfered.seq_groups
               ) == 0 and self.scheduler_config.policy == "priority":
            self._schedule_priority_preemption(budget,transfer_data)
        ...
        # Update new running requests.
        if len(transfered.seq_groups) > 0:
            # 首先将数据调度到传输队列
            self.transfered.extend([s.seq_group for s in transfered.seq_groups])

        # Merge lists
        num_prefill_groups = len(transfered.seq_groups)
        if num_prefill_groups > 0:
            scheduled_seq_groups = transfered.seq_groups

        ignored_seq_groups = transfered.ignored_seq_groups
        ...
        return SchedulerOutputs(
            ...
            ignored_seq_groups=ignored_seq_groups,
            num_lookahead_slots=0,
            running_queue_size=len(self.transfered),
            preempted=0,
        )


    def _shedule_disagg_test(self,transfer_data=False,transfer_queue=None,lock_list=None):
        ...
        # Update waiting requests.
        if not transfer_data and len(running_scheduled.preempted) != 0:
            self.waiting.extendleft(running_scheduled.preempted)
            with self.transfered_con:
                self.transfered_con.notify()
    ...
    def _shedule_disagg(self,transfer_data=False,transfer_queue=None):
        ...
        # If any requests are swapped, prioritized swapped requests.
        if not self.swapped:
            prefills = self._schedule_transfer_prefills(budget,
                                               curr_loras,
                                               enable_chunking=False)
    ...
    def _schedule_transfer_prefills(self,
        budget: SchedulingBudget,
        curr_loras: Optional[Set[int]],
        enable_chunking: bool = False,
        transfer_data: bool = False,
        ...
        while waiting_queue:
            ...
            self.set_transfer_running(seq_group)
    ...
    def scheduler_decode_transfer(self,transfer_data=False,transfer_queue=None):
        scheduler_start_time = time.perf_counter()
        
        scheduler_outputs: SchedulerOutputs = self._shedule_disagg_transfer(transfer_data,transfer_queue)
        ...
        for i, scheduled_seq_group in enumerate(
            ...
            for seq in seq_group.get_seqs(status=SequenceStatus.TRANSERED):
                seq_id = seq.seq_id
            ...
            if self.cache_config.enable_prefix_caching:
                common_computed_block_nums = (
                    self.block_manager.get_common_computed_block_ids(
                        seq_group.get_seqs(status=SequenceStatus.TRANSERED)))
    ...
    def schedule_decode(
                self,transfer_data=False,transfer_queue=None
        ...
        scheduler_outputs: SchedulerOutputs = self._shedule_disagg(transfer_data,transfer_queue)
    ...
    def schedule(
        ...
        # if transfer_data and len(self.waiting) == 0:
        #     # time.sleep(0.05)
        #     return (None,None,None)
    ...
    def _allocate_and_set_transfer_running(self, seq_group: SequenceGroup) -> None:
        self.block_manager.allocate(seq_group)
        for seq in seq_group.get_seqs(status=SequenceStatus.TRANSERED):
            seq.status = SequenceStatus.RUNNING
    
    def _allocate_and_set_transfer(self, seq_group: SequenceGroup) -> None:
        self.block_manager.allocate(seq_group)
        for seq in seq_group.get_seqs(status=SequenceStatus.WAITING):
            seq.status = SequenceStatus.TRANSERED

    def set_running(self, seq_group: SequenceGroup) -> None:
        for seq in seq_group.get_seqs(status=SequenceStatus.WAITING):
            seq.status = SequenceStatus.RUNNING

    def set_transfer_running(self, seq_group: SequenceGroup) -> None:
        for seq in seq_group.get_seqs(status=SequenceStatus.TRANSERED):
            seq.status = SequenceStatus.RUNNING
    
    def set_transfer(self, seq_group: SequenceGroup) -> None:
        for seq in seq_group.get_seqs(status=SequenceStatus.WAITING):
            seq.status = SequenceStatus.TRANSERED
